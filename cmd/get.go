...
var getEntityCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		filterKeys, _ := cmd.Flags().GetBool("filter")     // Get the filter flag
		...
		url := fmt.Sprintf("%s/api/catalog/entities/by-name/%s/%s/%s", baseUrl, kind, namespace, name) // Use ref for URL
...
func init() {
	getEntityCmd.Flags().StringP("ref", "r", "", "The reference of the entity")              // Add kind flag
	getEntityCmd.Flags().StringP("kind", "k", "", "The kind of the entity")                  // Add kind flag
	...
	getEntityCmd.Flags().BoolP("filter", "f", true, "Neat specific keys from the output")    // Add filter flag
